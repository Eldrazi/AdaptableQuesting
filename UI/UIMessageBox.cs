...
namespace AdaptableQuesting.UI
{
    public class UIMessageBox : UIPanel
    {
        ...
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            float textHeight = font.MeasureString("A").Y * scale;
            ...
            foreach (string line in lines)
            {
                ...
                {
                    string remainder = "";
                    while ((font.MeasureString(drawString).X * scale) > space.Width)
                    {
                    ...
                    {
                        Utils.DrawBorderString(spriteBatch, drawString, new Vector2(space.X, space.Y + position), Color.White, scale);
                    }
                    position += textHeight;
